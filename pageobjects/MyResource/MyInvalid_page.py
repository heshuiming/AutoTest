...
class MyInvalidPage(BasePage):
    ...
    #转出原因
    invalid_type0 = "xpath=>//a[@data-sertype='invalidType'][@data-serval='0']" #已过期
    invalid_type1 = "xpath=>//a[@data-sertype='invalidType'][@data-serval='1']" #转无效
    invalid_type2 = "xpath=>//a[@data-sertype='invalidType'][@data-serval='2']" #转公共资源
    def InvalidType(self,type):
        if type == 0:
            self.click(self.invalid_type0)
        elif type == 1:
            self.click(self.invalid_type1)
        elif type == 2:
            self.click(self.invalid_type2)
        else:
            print("请给出正确的转出原因!")
